...
style button:
    ...
    activate_sound "audio/button_bubble_click_new.ogg"
...
# 【新增】标题logo动画：淡入并向上滑动，带有脉动效果
transform title_logo_anim:
    alpha 0.0
    yoffset -50
    parallel:
...
# 注意：虽然可以在这里定义更多全局转场，但过多会影响游戏性能
# 建议在具体的场景切换中使用上述自定义变换
...
style window:
    ...
    # 先使用项目中已存在的文本框资源，避免因缺失自定义素材导致启动失败。
    background Frame("gui/textbox.png", gui.frame_borders, tile=gui.frame_tile)

transform dialog_box_appear:
    on show:
        alpha 0.0
        yoffset 20
        parallel:
...
## ====【主菜单屏幕修改说明】====
## 1. 添加了开场动画：背景音乐、淡入背景、标题logo动画
## 2. 重新设计了菜单布局：按钮居中，添加了动画效果
## 3. 添加了版本信息显示
## 4. 按钮样式自定义，包括悬停效果
## 5. 使用了自定义变换（Transforms）实现动画效果
...
screen main_menu():
    ...
    ## 使用现有主菜单背景，避免缺失自定义图片时报错。
    add "gui/main_menu.png" at fade_in
    ...
    vbox:
        ...
        textbutton _("开始游戏") action Start() style "main_menu_button" at slide_up
        textbutton _("继续游戏") action ShowMenu("load") style "main_menu_button" at slide_up
        textbutton _("游戏设置") action ShowMenu("preferences") style "main_menu_button" at slide_up
        textbutton _("关于游戏") action ShowMenu("about") style "main_menu_button" at slide_up
        textbutton _("退出游戏") action Quit(confirm=True) style "main_menu_button" at slide_up
...
style main_menu_button:
    ...
    xsize 300
    ysize 75
    background Frame("gui/button/idle_background.png", 25, 25)
    hover_background Frame("gui/button/hover_background.png", 25, 25)
    insensitive_background Frame("gui/button/idle_background.png", 25, 25)
    top_padding 15
    bottom_padding 15
    left_padding 50
    right_padding 50
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    ...
                    textbutton _("显示转场") action Preference("transitions", "toggle")
